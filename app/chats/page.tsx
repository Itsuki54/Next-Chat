...
export default function Chats() {
  ...
  const [messageText, setMessageText] = useState<Database["public"]["Tables"]["test"]["Row"][]>([])
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const { data } = await supabase.from("test").select("*").eq('channel', channelName).order("created_at")
  ...
  const onSubmitNewMessage = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await supabase.from("test").insert({ message: inputText, uid: userID, channel: channelName, name: userName })
    } catch (error) {
      console.error(error)
    }
    setInputText("")
